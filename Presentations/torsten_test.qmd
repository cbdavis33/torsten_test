...
model <- cmdstan_model(
  here::here("depot_1cmt_linear/Stan/Simulate/depot_1cmt_exp.stan"))
...
mod <- mread(here::here("depot_1cmt_linear/mrgsolve/depot_1cmt_exp.cpp"))
...
So you can see that the analytical solution spits out a vector of 0s, while the
linear ODE, general ODE, and mrgsolve simulations are all the same.
...
model <- cmdstan_model(
  here::here("iv_2cmt_linear/Stan/Simulate/iv_2cmt_ppa.stan"))
...
mod <- mread(here::here("iv_2cmt_linear/mrgsolve/iv_2cmt_ppa.cpp"))
...
model_analytical <- cmdstan_model(
  here::here("example_models_metrum/pk2cpt/pk2cpt.stan"))

fit_analytical <- model_analytical$sample(
  data = here::here("example_models_metrum/pk2cpt/pk2cpt.data.R"), 
  init = here::here("example_models_metrum/pk2cpt/pk2cpt.init.R"), 
  seed = 3191951,
  chains = 1,
  parallel_chains = 1,
  iter_warmup = 500,
...
In particular, 
```Chain 1 Exception: normal_lpdf: Location parameter[1] is -inf, but must be finite!```
implies that the prediction is 0.
...
model_mat_exp <- cmdstan_model(
  here::here("example_models_metrum/pk2cpt_linode/pk2cpt_linode.stan"))

fit_mat_exp <- model_mat_exp$sample(
  data = here::here("example_models_metrum/pk2cpt_linode/pk2cpt_linode.data.R"), 
  init = here::here("example_models_metrum/pk2cpt_linode/pk2cpt_linode.init.R"), 
  seed = 3191951,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 500,
...
Works fine. I also fit the ```pk2cpt``` and ```pk2cpt_ode``` data with this 
model with no issues.
...
model_rk45 <- cmdstan_model(
  here::here("example_models_metrum/pk2cpt_ode/pk2cpt_ode.stan"))

stan_data_rk45 <- rstan::read_rdump(
  here::here("example_models_metrum/pk2cpt_ode/pk2cpt_ode.data.R"))

stan_data_rk45$prior_only <- 1

fit_rk45 <- model_rk45$sample(
  data = stan_data_rk45, 
  init = here::here("example_models_metrum/pk2cpt_ode/pk2cpt_ode.init.R"), 
  seed = 3191951,
  chains = 1,
  parallel_chains = 1,
  iter_warmup = 1,
...
As you can see, it's returning a vector of 0s as the solution.
...
model <- cmdstan_model(
  here::here("example_models_metrum/poppk2cpt/poppk2cpt.stan"))

stan_data <- rstan::read_rdump(
  here::here("example_models_metrum/poppk2cpt/poppk2cpt.data.R"))

stan_data$solver <- 1

fit_analytical <- model$sample(
  data = stan_data, 
  init = here::here("example_models_metrum/poppk2cpt/poppk2cpt.init.R"), 
  seed = 3191951,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 500,
...
model <- cmdstan_model(
  here::here("depot_2cmt_linear/Stan/Fit/depot_2cmt_prop_all_solvers.stan"),
